...
ntp_time_t us_to_ntp_time(uint64_t micros) {
    uint32_t seconds    = micros / 1000000;
    ntp_time_t ntp_time = {.fraction = (uint32_t)(((uint64_t)(UINT32_MAX) *
                                                   (micros % 1000000ull)) /
                                                  1000000ull),
                           .seconds  = seconds};
...
int64_t get_server_time(SPI_DEVICE_PARAM, ip_t server_addr,
    ...
    ntp_time_t t0_ntp = us_to_ntp_time(t0);
    packet.org        = t0_ntp;
    ...
    uint64_t server_time_center          = t2;
    ...
    // printf("t3: %lld\n", t3);  // t0 and t3 return proper numbers (system
    // time since start, but called at different times) printf("%lld  -  ", t1 +
    // t2); printf("%lld  = 2*  ", t0 + t3);
