...
bool tcp_server_connected(tcp_server_t *server) {
  return w5500_read8(server->w5500, server->socket, W5500_Sn_SR) == W5500_SOCK_ESTABLISHED;
}

tcp_client_data_t tcp_server_get_client(tcp_server_t *server) {
  tcp_client_data_t client;
  w5500_rw(server->w5500, server->socket, W5500_Sn_DIPR0, false, client.ip, sizeof(client.ip));
